...
Autograd 现在是 torch 自动微分的核心包 .
它是使用基于 tape 的系统来进行自动微分的.
...
还有一个对于 autograd 的使用非常重要的类 -  ``Function`` 类. ``Variable``
和 ``Function`` 是相互关联的, 并创建了一张无环图, 它记录一个完整的计算历史.
每个 ``Variable`` 的 ``.grad_fn`` 属性都引用了一个计算出这个Variable的函数
(除了用户创建的变量外 - 这些变量的 ``.grad_fn`` 为 ``None`` ).

如果你想要计算导数, 你可以在 ``Variable`` 上调用 ``.backward()``.
如果 ``Variable`` 是一个标量 (i.e. 它拥有一个tensor元素),
则不需要为 ``backward()`` 指定任何参数, 但是如果它包含许多的元素,
...
###############################################################
# 在默认情况下, 梯度计算会刷新计算图中包含的所有内部缓冲区,
# 所以如果您想要在图的某个部分向后执行两次梯度计算,则需要在
# 第一次传递过程中设置参数为 ``retain_variables = True``.
