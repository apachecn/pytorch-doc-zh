...
######################################################################
# 可以查阅 `文档 <http://pytorch.org/docs/torch.html>`__ 获取大量可用操作的完整列表, 扩展到了非数学操作. 
#
# 接下来一个很有帮助的操作就是连接.
#
...
# ~~~~~~~~~~~~~~~~~
#
# 使用.view()去重构tensor.这是一个高频方法, 因为许多神经网络的神经元对输入格式
# 有明确的要求. 你通常需要先将数据重构再输入到神经元中.
...
# autograd.Variable.
#
# 首先, 从程序员的角度来思考.在torch中存储了什么, 是我们在上面创建的Tensor对象吗? 显然, 是数据和
# 结构, 也很可能是其他的东西. 但是当我们将两个tensors相加后, 我们得到了一个输出tensor.这个输出所能
# 体现出的只有数据和结构, 并不能体现出是由两个tensors加和得到的(因为它可能是从一个文件中读取的, 也可能是
# 其他操作的结果等).
#
...
######################################################################
# 既然变量知道怎么创建的它们. z 知道它并非是从文件读取的, 也不是乘法或指数或其他运算的结果.
# 如果你继续跟踪z.grad_fn, 你会从中找到x和y的痕迹. 
#
# 但是它如何帮助我们计算梯度?
#
...
# 程序的开发人员用程序指令sum()和 + 操作以知道如何计算它们的梯度并且运行反向传播算法.深入讨论此算法
# 超出了本教程的范围.
#
...
# 怎么会这样? 我们将tensor从var_z中提取 (提取为var_z.data).这个张量不知道它是如
# 何计算的.我们把它传递给new_var_z.
# 这就是new_var_z得到的所有信息. 如果var_z_data不知道它是如何计算的, 那么就不会 
# 有new_var_z的方法.
