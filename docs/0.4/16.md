...
$$ 
\begin{aligned} 
D_{out}&=(D_{in}-1)_stride[0]-2_padding[0]+kernel_size[0]\\
H_{out}&=(H_{in}-1)_stride[1]-2_padding[0]+kernel_size[1]\\
W_{out}&=(W_{in}-1)_stride[2]-2_padding[2]+kernel_size[2]
\end{aligned} 
$$
...
$$
\begin{aligned} 
out(N_i,C_j,h,w) = 1/(kH_kW)\sum^{kH-1}_{m=0}\sum^{kW-1}_{n=0}input(N_{i},C_{j},stride[0]_h+m,stride[1]_w+n)
\end{aligned} 
$$
...
$$
\begin{aligned} 
H_{out}=floor((H_{in}+2*padding[0]-kernel_size[0])/stride[0]+1)\\
W_{out}=floor((W_{in}+2*padding[1]-kernel_size[1])/stride[1]+1) 
\end{aligned} 
$$
...
$$ 
\begin{aligned} 
out(N_i,C_j,d,h,w)=1/(kD_kH_kW)*\sum^{kD-1}_{k=0}\sum^{kH-1}_{m=0}\sum^{kW-1}_{n=0}input(N_{i},C_{j},stride[0]_d+k,stride[1]_h+m,stride[2]*w+n)
\end{aligned} 
$$ 
如果`padding`不是0，会在输入的每一边添加相应数目0
