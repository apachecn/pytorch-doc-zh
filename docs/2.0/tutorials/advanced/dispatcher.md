...
确定您调用运算符时
最高优先级的调度键是什么(这是通过查看tensor参数以及
某些线程本地状态来完成的)，并将控制权转移到内核
调度钥匙。最终效果是，当您调用运算符时，我们首先执行 Autograd 内核，然后根据传入tensor的设备类型重新分派到后端内核。
...
注册内核以在所有情况下运行，即使tensor不是 CPU
tensor！ (在内部，我们将这些称为 “catch-all” 内核，因为它们
c捕获所有情况。)为了确保 
 `myadd_cpu`
 仅运行于\ nCPU tensor，我们可以使用
 `TORCH_LIBRARY_IMPL`
 宏：
...
2.上面注册的
 ...
 `ADInplaceOrView`
 盒装内核
依赖于其日志中的操作员架构信息。如果您的操作就地改变输入或返回与其中一个输入别名的tensor，请务必确保您的架构正确反映这一点。有关如何注释架构的更多信息，请参阅
...
在运行运算之前
达到某个首选精度。
例如，浮点 CUDA tensor上的 matmul 和卷积通常运行得更快
并且在
 `float16 中使用更少的内存`
 不会影响收敛。
Autocast 包装器仅在
...
可能在
 `float16`
 中执行，以及
* 其他操作具有多个浮点tensor输入应该将它们标准化为通用精度(除非实现支持不同精度的输入)。
