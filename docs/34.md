...
我们的目标是训练出一个机制, 尽可能做到最大化折扣因子和累积回馈因子. `\(R_{t_0} = \sum_{t=t_0}^{\infty} \gamma^{t - t_0} r_t\)`, `\(R_{t_0}\)` 也被称为 _回馈因子_. 折扣因子 `\(\gamma\)` 应该是一个 位于 `\(0\)` 和 `\(1\)` 之间的常量, 且确保了其总和是收敛的. 它的存在意义是让 回馈的重要程度与时间成正比, 即离现在1分钟的回馈比离现在1小时的回馈要更重要, 因为离当前时间越近, 我们的预测值可以更准确, 更可信.

Q-learning 的主要原理是, 假如我们有一个函数, `\(Q^*: State \times Action \rightarrow \mathbb{R}\)`, 这将会定义返回值, 如果我们在给定状态下做出动作, 那么我们将会更容易据此训练出一个机制, 并做到最大化回馈因子.

```py
\[\pi^*(s) = \arg\!\max_a \ Q^*(s, a)\]
```

由于我们对整个环境一无所知, 我们不需要知道确定的 `\(Q^*\)`. 但是, 因为神经网络 是一个泛化的逼近函数, 所以我们可以直接构造一个网络并训练它去模拟 :math:[`](#id2)Q^*`即可.

对于我们训练的更新规则来说, 我们只需要让每一个 `\(Q\)` 遵从贝尔曼方程 (Bellman equation) 就可以了.

```py
\[Q^{\pi}(s, a) = r + \gamma Q^{\pi}(s', \pi(s'))\]
```

方程两边的实际差值即为时间差分误差 (temporal difference error), `\(\delta\)`:

```py
\[\delta = Q(s, a) - (r + \gamma \max_a Q(s', a))\]
```

为了使得该误差值最小化, 我们要使用 [Huber loss](https://en.wikipedia.org/wiki/Huber_loss). 当时间差分误差较小时, Huber loss 表现地与均方误差 (mean squared error) 一样, 而当时间差分误差较大时, Huber loss 表现地与绝对均差 (mean absolute error) 一样. 这一性质使得它在预测带有较多噪音的 `\(Q\)` 值上更具有鲁棒性. 我们通过从重播记忆中取出一批样本来计算 `\(B\)`.
...
我们的模型是一个卷积神经网络 (CNN), 将当前屏幕图与之前屏幕图的差值作为唯一输入, 输出值有两个, 分别代表 `\(Q(s, \mathrm{left})\)` 和 `\(Q(s, \mathrm{right})\)` (其中 `\(s\)` 是网络的输入). 从效果上来看, 我们的神经网络模型可以预测在当前输入下采取特定行为带来的 _quality_, 即对整个环境的影响.
...
在下面代码中有一个 `optimize_model` 函数, 它用于实现优化过程的其中一步. 它首先 取出一个批次的样本, 然后将所有的张量全部合并到一个中, 并计算 `\(Q(s_t, a_t)\)` 和 `\(V(s_{t+1}) = \max_a Q(s_{t+1}, a)\)`, 最终将这些结果全都融入到loss中去. 假如 `\(s\)` 是一个终止状态, 则 `\(V(s) = 0\)`.
...
下面的 &lt;cite&gt;num_episodes&lt;/cite&gt; 变量设置的很小. 你可以把这个 notebook 下载下来然后运行更多帧.
