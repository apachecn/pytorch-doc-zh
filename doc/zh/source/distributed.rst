...
目前torch.distributed支持4个后端, 每个都有不同的功能. 下表显示哪些功能可用于 CPU/CUDA 张量.
...
:func:`~torch.distributed.all_gather_multigpu` 
支持每个节点内多个GPU之间的分布式集体操作。 这些功能可以提高整体分布式训练效果，并通过传递Tensor列表来轻松使用。
传递的Tensor列表中的每个Tensor需要位于调用函数的主机的单独GPU设备上。 请注意，Tensor列表的长度在所有分布式进程中需要相同。 
另请注意，目前多GPU集合函数仅支持NCCL后端。

例如，如果我们用于分布式训练的系统有2个节点，每个节点有8个GPU。 
在16个GPU中，每个都有一个我们希望归约的Tensor。 以下代码可以作为参考:
...
调用之后，两个节点上的所有16个Tensor将具有16的规约
...
--------------
 ...
 `torch.distributed` 包在`torch.distributed.launch`中同样提供了一个启动实用程序.
