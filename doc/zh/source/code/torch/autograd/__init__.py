...
def backward(variables, grad_variables=None, retain_graph=None, create_graph=None, retain_variables=None):
    ...
    它的长度应该和variables的长度匹配, 里面保存了相关variable的梯度 (对于不需要gradient tensor的variable，应制定为None).

    此函数累积叶子节点variables计算的梯度 - 调用此函数之前应先将叶子节点variables梯度置零.

    参数说明:
        variables (Variable列表): 被求微分的叶子节点.
        grad_variables ((Tensor, Variable or None)列表): 对应variable的梯度.  任何张量将自动转换为变量除非
...
def grad(outputs, inputs, grad_outputs=None, retain_graph=None, create_graph=None,
    ...
    参数说明:
        ...
        grad_outputs (张量或变量序列): 每一个输出的梯度.
            ...
            如果所有grad_variables都可以接受None值，那么这个参数是可选的.
        retain_graph (bool, 可选): 如果是``False``, 用于计算grad的图将被释放. 几乎所有情况都设置为``True``
        ...
        allow_unused (bool, 可选): 如果是``False``, 指定计算输出时未使用的输入（因此它们的grad始终为零）是错误的. 默认为``False``.
    """
