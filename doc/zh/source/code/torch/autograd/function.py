...
class _ContextMethodMixin(object):
    ...
    def save_for_backward(self, *tensors):
        ...
        **T这个方法至多只能被调用一次, 只能在**
        ...
        属性获取; 或者, 如果变量还需要被使用 (比如二次求导), 能够被参数:attr:`saved_variables`获取.  Before returning them to the user, a check is made, to ensure
        保证这些tensor没有被in-place operations修改过.
    ...
    def mark_dirty(self, *args):
        ...
        **这个方法应当至多调用一次, 只能在方法**
        :func:`forward` **中用, 实参只能是forward的实参.**

        每个在forward方法中被in-place operations修改的tensor都应该传递给这个方法。
        这样，可以保证检查的正确性。这个方法在tensor修改前后调用都可以。
        """
    ...
    def mark_shared_storage(self, *pairs):
        ...
        **这个方法应当至多调用一次, 只能在方法**
        ...
        保证 in-place operations 检查的正确性。唯一的特例就是，当 output和input是同一个tensor(in-place operations的输入和输出)。
    ...
    def mark_non_differentiable(self, *args):
        ...
        **这个方法至多只能被调用一次,只能在方法*
        :func:`forward` **中用, 而且实参只能是forward的返回值.**

        这个方法会将输出标记成不可微，会增加backward过程中的效率。在backward中，
        你依旧需要接收forward输出值的梯度，但是这些梯度一直是None。.

        这用于例如 对于从最大值返回的索引:class:`Function`.
        """
...
class Function(with_metaclass(FunctionMeta, _C._FunctionBase, _ContextMethodMixin, _HookMixin)):
    """记录操作历史记录并定义区分操作的方法。
     ...
     每个执行在Varaibles上的operation都会创建一个Function对象，这个Function对象执行计算工作，同时记录下来。这个历史以有向无环图的形式保存下来，
     有向图的节点为functions，有向图的边代表数据依赖关系(input<-output)。之后，当backward被调用的时候，计算图以拓扑顺序处理，通过调用每个Function对象的backward()，
...
def traceable(fn_cls):
    ...
    可追踪函数有其他限制 - 它们不能传递任何依赖于数据的值到后向（例如Prod传递输出，这使得它不可追踪），
    并且它们的导数应该完全在autograd变量的所有情况下执行 （即使导数是不稳定的）。
